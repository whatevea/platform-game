class Inventory {
  ...
  renderSlots() {
    for (let i = 0; i < this.limit; i++) {
      ...
      graphics.strokeRect((i * 40) + 2, this.y_location + 2, 40, 42)
    }
  }
  addSlot() {
    //append the this.storage
    this.limit++;
    this.storage.push({ "filled": false, "item": "", "sprite": "" })
    //append the box
    this.renderSlots();
  ...
  getBlankLocation() {
    return this.storage.findIndex(item => !item.filled);
  } ///wasted 30 min on this single line

  getItemLocation(type) {
    return this.storage.findIndex(item => type == item["item"]);
  }
    ...
    .then((value) => {
      switch (value) {
        ...
        case "use":
          items_json[item].use();
          inventory.removeItem(location);
          swal({ text: items_json[item].usedText})
          break;
      }
    });
